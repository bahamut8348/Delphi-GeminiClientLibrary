unit Parameters.AsyncBatchEmbedContentRequestBody;
...
uses
  ...
  Parameters.BasedParameterImplement, Parameters.InputEmbedContentConfig,
  Parameters.EmbedContentBatchOutput, Parameters.EmbedContentBatchStats;

type
{
  https://ai.google.dev/api/embeddings#request-body_2
  ...
  TAsyncBatchEmbedContentRequestBody = class(TParameterReality)
  private
    { private declarations }
    FName: String;
    FDisplayName: String;
    FInputConfig: TInputEmbedContentConfig;
    FOutput: TEmbedContentBatchOutput;
    FCreateTime: String; // (Timestamp format)
    FEndTime: String; // (Timestamp format)
    FUpdateTime: String; // (Timestamp format)
    FBatchStats: TEmbedContentBatchStats;
    FState: String;
    FPriority: String;

    procedure SetBatchStats(const Value: TEmbedContentBatchStats);
    procedure SetCreateTime(const Value: String);
    procedure SetDisplayName(const Value: String);
    procedure SetEndTime(const Value: String);
    procedure SetInputConfig(const Value: TInputEmbedContentConfig);
    procedure SetName(const Value: String);
    procedure SetOutput(const Value: TEmbedContentBatchOutput);
  ...
  public
    ...
    property inputConfig: TInputEmbedContentConfig read FInputConfig write SetInputConfig;
    // batch.output 仅限输出。批量请求的输出。
    property output: TEmbedContentBatchOutput read FOutput write SetOutput;
    ...
    property batchStats: TEmbedContentBatchStats read FBatchStats write SetBatchStats;
  ...
  public
    ...
    class function CreateWith(const szDisplayName: String; const pInputConfig: TInputEmbedContentConfig;
      const szPriority: String = '0'): TAsyncBatchEmbedContentRequestBody; inline; static;
  published
...
{ TAsyncBatchEmbedContentRequestBody }

constructor TAsyncBatchEmbedContentRequestBody.Create();
begin
  inherited Create();
  Self.FName := '';
  Self.FDisplayName := '';
...
class function TAsyncBatchEmbedContentRequestBody.CreateWith(
  const szDisplayName: String; const pInputConfig: TInputEmbedContentConfig;
  const szPriority: String): TAsyncBatchEmbedContentRequestBody;
begin
  Result := TAsyncBatchEmbedContentRequestBody.Create();
  Result.displayName := szDisplayName;
  Result.inputConfig := pInputConfig;
  Result.priority := szPriority;
end;

destructor TAsyncBatchEmbedContentRequestBody.Destroy();
begin
  Self.FPriority := '';
  Self.FState := '';
...
function TAsyncBatchEmbedContentRequestBody.GetMemberValue(var sName: String;
...
procedure TAsyncBatchEmbedContentRequestBody.SetBatchStats(
  const Value: TEmbedContentBatchStats);
begin
  if (Value <> FBatchStats) then
    TParameterReality.CopyWithClass(FBatchStats, Value);
end;

procedure TAsyncBatchEmbedContentRequestBody.SetCreateTime(const Value: String);
begin
  FCreateTime := Value;
end;

procedure TAsyncBatchEmbedContentRequestBody.SetDisplayName(
  const Value: String);
begin
  FDisplayName := Value;
end;

procedure TAsyncBatchEmbedContentRequestBody.SetEndTime(const Value: String);
begin
  FEndTime := Value;
end;

procedure TAsyncBatchEmbedContentRequestBody.SetInputConfig(
  const Value: TInputEmbedContentConfig);
begin
  if (Value <> FInputConfig) then
    TParameterReality.CopyWithClass(FInputConfig, Value);
end;

function TAsyncBatchEmbedContentRequestBody.SetMemberValue(const sName: String;
...
begin
  ...
  begin
    TParameterReality.CopyWithJson(FInputConfig, TInputEmbedContentConfig, pValue);
    Result := TRUE;
  end
  else if SameText(sName, 'batch.output') then
  begin
    TParameterReality.CopyWithJson(FOutput, TEmbedContentBatchOutput, pValue);
    Result := TRUE;
  end
  ...
  begin
    TParameterReality.CopyWithJson(FBatchStats, TEmbedContentBatchStats, pValue);
    Result := TRUE;
  end
...
procedure TAsyncBatchEmbedContentRequestBody.SetName(const Value: String);
begin
  FName := Value;
end;

procedure TAsyncBatchEmbedContentRequestBody.SetOutput(
  const Value: TEmbedContentBatchOutput);
begin
  if (Value <> FOutput) then
    TParameterReality.CopyWithClass(FOutput, Value);
end;

procedure TAsyncBatchEmbedContentRequestBody.SetPriority(const Value: String);
begin
  FPriority := Value;
end;

procedure TAsyncBatchEmbedContentRequestBody.SetState(const Value: String);
begin
  FState := Value;
end;

procedure TAsyncBatchEmbedContentRequestBody.SetUpdateTime(const Value: String);
begin
  FUpdateTime := Value;
end;
