unit Parameters.GenerateContentRequest;
...
type
{
  ...
  TGenerateContentRequest = class(TParameterReality)
  private
...
{ TGenerateContentRequest }

constructor TGenerateContentRequest.Create();
begin
  inherited Create();
  Self.FModel := '';
...
destructor TGenerateContentRequest.Destroy();
begin
  Self.FCachedContent := '';
...
function TGenerateContentRequest.GetMemberValue(var sName: String;
  out pValue: TValue): Boolean;
begin
  Result := inherited GetMemberValue(sName, pValue);
end;

procedure TGenerateContentRequest.SetCachedContent(const Value: String);
begin
  FCachedContent := Value;
end;

procedure TGenerateContentRequest.SetContents(const Value: TArray<TContent>);
begin
  TParameterReality.CopyArrayWithClass<TContent>(FContents, Value);
end;

procedure TGenerateContentRequest.SetGenerationConfig(
...
function TGenerateContentRequest.SetMemberValue(const sName: String;
