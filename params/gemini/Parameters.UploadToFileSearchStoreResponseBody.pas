unit Parameters.UploadToFileSearchStoreResponseBody;
...
type
{
  https://ai.google.dev/api/file-search/file-search-stores#response-body
  这是 google.longrunning.Operation 的副本。我们需要复制它，因为为了与 Scotty 互动，我们需要添加一个无法在顶级 Operation proto 中添加的 Scotty 特定字段。
}
  TUploadToFileSearchStoreResponseBody = class(TParameterReality)
  private
    { private declarations }

    // 联合字段的序号，result 只允许存在一种值，如果全部都赋值发送给接口会返回错误。
  ...
  public
    ...
    property metadata: TObject read FMetadata write SetMetadata;
    // 如果值为 false，则表示操作仍在进行中。如果为 true，则表示操作已完成，其结果不是 error 就是 response。
    property done: Boolean read FDone write FDone;

    { Union type result start }
    // 操作结果，可以是 error，也可以是有效的 response。如果 done == false，则既不会设置 error，也不会设置 response。如果 done == true，则只能设置 error 或 response 中的一项。部分服务可能不会提供结果。result 只能是下列其中一项：
    ...
    // 此对象可以包含任意类型的字段。附加字段 "@type" 包含用于标示相应类型的 URI。示例：{ "id": 1234, "@type": "types.example.com/standard/id" }。
    property response: TObject read FResponse write SetResponse;
    { Union type result end }
...
{ TUploadToFileSearchStoreResponseBody }

constructor TUploadToFileSearchStoreResponseBody.Create();
begin
  inherited Create();
...
destructor TUploadToFileSearchStoreResponseBody.Destroy();
begin
...
function TUploadToFileSearchStoreResponseBody.GetMemberValue(var sName: String;
  out pValue: TValue): Boolean;
begin
  if SameText(sName, 'error') then
  begin
    if (upError = FUnionProperty) then
      pValue := TValue.From(Self.FError)
    else
      pValue := TValue.Empty;
    Result := TRUE;
  end
  ...
  begin
    if (upResponse = FUnionProperty) then
      pValue := TValue.From(Self.FResponse)
    else
      pValue := TValue.Empty;
    Result := TRUE;
  end
...
procedure TUploadToFileSearchStoreResponseBody.SetError(const Value: TStatus);
begin
...
function TUploadToFileSearchStoreResponseBody.SetMemberValue(
  const sName: String; const pValue: TJsonValue): Boolean;
begin
  if SameText(sName, 'metadata') then
  begin
    TParameterReality.CloneWithClass(Self.FMetadata, Self.FMetadataNeedFree, pValue);
    Result := TRUE;
  end
  ...
  begin
    ...
    TParameterReality.CloneWithJson(Self.FResponse, Self.FResponseNeedFree, pValue);
    Result := TRUE;
  end
  else
...
procedure TUploadToFileSearchStoreResponseBody.SetMetadata(
  const Value: TObject);
begin
...
procedure TUploadToFileSearchStoreResponseBody.SetResponse(
  const Value: TObject);
begin
  FUnionProperty := upResponse;
